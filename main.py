...
async def handle_format_selection(event):
    ...
    if COOKIES_IN_MEMORY:
        ydl_opts["cookiefile"] = COOKIES_IN_MEMORY
...
async def receive_cookies(event):
    global COOKIES_IN_MEMORY
    if event.file.name.lower() == "cookies.txt":
        # Save cookies in memory instead of disk
        COOKIES_IN_MEMORY = await event.download_media(file=bytes)
        await event.respond("✅ `cookies.txt` updated successfully and stored in memory!")
    else:
